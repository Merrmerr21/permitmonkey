# agents-permitmonkey — Development Prompt

You are building the **Agent SDK backend** for PermitMonkey — an AI ADU permit assistant that helps contractors respond to city corrections letters. Work is organized into **phases** — complete all tasks in a phase, then stop for verification.
...
**Key Files**:
- `agents-permitmonkey/claude-task.json` — Phases and tasks (your roadmap)
...
1. **Read `agents-permitmonkey/claude-task.json`** — Find the current phase (first one where `status` is not `"complete"`)
...
### Git Commits
After each task, stage only the agents-permitmonkey/ and test-assets/ directories (NOT the parent repo):
```bash
git add agents-permitmonkey/ test-assets/ && git commit -m "task-XXX: Brief description"
```

### Marking Progress
When a task is done, update `agents-permitmonkey/claude-task.json`:
...
```
agents-permitmonkey/                   ← Agent SDK project root (cwd for query() calls)
├── .claude/
...
const result = query({
  ...
  options: {
    ...
    systemPrompt: {
      type: 'preset',
      preset: 'claude_code',
      append: PERMITMONKEY_SYSTEM_PROMPT,                          // Our custom context
    },
    cwd: AGENTS_ROOT,                                           // agents-permitmonkey/ — NOT the parent project
...
| Wrong `cwd` | Loads 13 skills from parent (nano-banana, etc.) | Point to `agents-permitmonkey/`, not `permitmonkey/` |
...
- **Claude Code CLI** (us building the app) — runs from `permitmonkey/`, sees ALL 13 skills
- **Agent SDK** (the app running the pipeline) — runs from `agents-permitmonkey/`, sees ONLY 6 ADU skills
- Symlinks in `agents-permitmonkey/.claude/skills/` point to source of truth in `adu-skill-development/skill/`
...
4. Check `agents-permitmonkey/claude-task.json` for task details
5. Ask the user for clarification

---

**Now read `agents-permitmonkey/claude-task.json`, find the current phase, and begin working through its tasks.**
