...
category: operations
relevance: "When navigating the PermitMonkey UI via Chrome browser automation"
---

# PermitMonkey UI Navigation
...
Three modes stored in `localStorage` key `permitmonkey-app-mode`:
...
```javascript
localStorage.setItem('permitmonkey-app-mode', 'dev-test')
