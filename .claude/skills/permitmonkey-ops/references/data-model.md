...
# PermitMonkey Data Model

**Supabase Project:** bhjrpklzqyrelnhexhlj
**Schema:** `permitmonkey` (NOT `public` — always use `.schema('permitmonkey')` or `SET search_path TO permitmonkey;`)
...
| Bucket | Contents |
|--------|----------|
| `permitmonkey-demo-assets` | Pre-loaded demo files (plan binder, corrections PNGs, pre-built archives) |
| `permitmonkey-uploads` | User-uploaded files + Cloud Run extraction archives |
| `permitmonkey-outputs` | Agent-generated PDFs and artifacts |

### Archive Files

Pre-extracted page PNGs and title block crops are stored as `.tar.gz` archives. These are created either:
- **Demo projects:** Pre-built and uploaded to `permitmonkey-demo-assets/placentia/`
- **Real projects:** Created by Cloud Run extraction service, uploaded to `permitmonkey-uploads/`
...
SELECT id, status, flow_type, error_message FROM permitmonkey.projects WHERE id = 'PROJECT_ID';

-- Get latest messages
SELECT role, content, created_at FROM permitmonkey.messages
...
FROM permitmonkey.outputs WHERE project_id = 'PROJECT_ID' ORDER BY created_at DESC LIMIT 1;

-- Get contractor answers
SELECT question_key, question_text, answer_text, is_answered
FROM permitmonkey.contractor_answers WHERE project_id = 'PROJECT_ID';

-- List all demo projects
SELECT id, project_name, flow_type, status, city FROM permitmonkey.projects WHERE is_demo = true;
```
