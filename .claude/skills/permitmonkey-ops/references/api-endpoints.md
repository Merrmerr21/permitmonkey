...
# PermitMonkey API Endpoints

All endpoints accept `Authorization: Bearer $PERMITMONKEY_API_KEY` header. Browser users authenticate via Supabase session cookies instead.

**Base URL (production):** `https://cc-permitmonkey.vercel.app`
...
while true; do
  STATUS=$(curl -s https://cc-permitmonkey.vercel.app/api/projects/$PROJECT_ID \
    -H "Authorization: Bearer $PERMITMONKEY_API_KEY" | jq -r '.project.status')
  echo "Status: $STATUS"
...
```bash
# All runs
curl -s https://cc-permitmonkey.vercel.app/api/projects/b0000000-0000-0000-0000-000000000002/runs \
  -H "Authorization: Bearer $PERMITMONKEY_API_KEY" | jq '.runs[] | {version, flow_phase, agent_cost_usd, created_at}'

# Only analysis runs
curl -s "https://cc-permitmonkey.vercel.app/api/projects/b0000000-0000-0000-0000-000000000002/runs?flow_phase=analysis" \
  -H "Authorization: Bearer $PERMITMONKEY_API_KEY"
```
...
5. Uploads archives to Supabase Storage (`permitmonkey-uploads` bucket)
...
**Example:**
```bash
curl -X POST https://cc-permitmonkey.vercel.app/api/extract \
  -H "Authorization: Bearer $PERMITMONKEY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"project_id":"b0000000-0000-0000-0000-000000000002"}'
```
...
**Base URL:** `https://permitmonkey-server-v7eqq3533a-uc.a.run.app`

### GET /health
```bash
curl https://permitmonkey-server-v7eqq3533a-uc.a.run.app/health
# → {"status":"ok"}
```
...
```bash
curl -X POST https://permitmonkey-server-v7eqq3533a-uc.a.run.app/extract \
  -H "Content-Type: application/json" \
  -d '{"project_id":"b0000000-0000-0000-0000-000000000002"}'
```
