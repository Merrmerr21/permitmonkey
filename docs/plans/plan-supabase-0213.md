# PermitMonkey — Supabase Schema Plan (Feb 13, 2026)

> **Purpose:** Single source of truth for the PermitMonkey database schema.
> **Project:** Labyrinth (`bhjrpklzqyrelnhexhlj`) on us-east-1
> **Schema:** `permitmonkey` (DROP and rebuild fresh — old schema is from a different iteration)

---

## The Two Flows (Why This Matters for Schema)

PermitMonkey has two distinct flows that share infrastructure but have different lifecycles:
...
```sql
-- ============================================================
-- PermitMonkey Schema — Fresh rebuild
-- ============================================================

-- Nuke old schema (old iteration, incompatible structure)
DROP SCHEMA IF EXISTS permitmonkey CASCADE;
CREATE SCHEMA permitmonkey;
...
CREATE TABLE permitmonkey.contractor_answers (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  project_id UUID REFERENCES permitmonkey.projects(id) ON DELETE CASCADE NOT NULL,
...
CREATE POLICY "Users can CRUD own contractor answers"
  ON permitmonkey.contractor_answers FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM permitmonkey.projects p
      WHERE p.id = project_id AND (p.user_id = auth.uid() OR p.is_demo = true)
...
|---|---|---|
| `permitmonkey-uploads` | User-uploaded plan binders + corrections letters | Authenticated users |
| `permitmonkey-outputs` | Agent-generated files (PDFs, response packages) | Authenticated users |
| `permitmonkey-demo-assets` | Pre-seeded demo files (plan binder, corrections letter) | Public read |
...
- Create judge account: `judge@permitmonkey.app` / `permitmonkey-hackathon-2026`
...
-- Link demo files (after uploading PDFs to permitmonkey-demo-assets bucket)
INSERT INTO permitmonkey.files (project_id, file_type, filename, storage_path, mime_type)
VALUES
  ('a0000000-0000-0000-0000-000000000001', 'plan-binder', 'plan-binder.pdf', 'permitmonkey-demo-assets/buena-park/plan-binder.pdf', 'application/pdf'),
  ('a0000000-0000-0000-0000-000000000002', 'plan-binder', 'plan-binder.pdf', 'permitmonkey-demo-assets/buena-park/plan-binder.pdf', 'application/pdf'),
  ('a0000000-0000-0000-0000-000000000002', 'corrections-letter', 'corrections-letter.pdf', 'permitmonkey-demo-assets/buena-park/corrections-letter.pdf', 'application/pdf');
```
