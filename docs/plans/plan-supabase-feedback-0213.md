...
1. Judge clicks "Analyze Corrections" on contractor demo project
   ...
   └→ Agent streams messages to permitmonkey.messages
   └→ Skill 1 finishes → produces contractor_questions.json
   └→ Cloud Run parses contractor_questions.json
   └→ Cloud Run inserts questions into permitmonkey.contractor_answers
...
2. Judge answers the contractor questions in the UI
   └→ Frontend updates permitmonkey.contractor_answers rows
...
2. Add the permitmonkey.contractor_answers table
...
The entire agents-permitmonkey/ codebase is file-based. It writes everything to local sessions/ directories as JSON and Markdown files. There's no Supabase client, no .from(), no .insert() -- nothing. The only env var it needs is ANTHROPIC_API_KEY.
...
projects.user_id references auth.users(id) directly -- correct. The old schema had a separate permitmonkey.users table. You don't need that anymore.
...
-- Add to permitmonkey.outputs:
raw_artifacts JSONB DEFAULT '{}'::jsonb,
-- Add to permitmonkey.outputs:raw_artifacts JSONB DEFAULT '{}'::jsonb,
