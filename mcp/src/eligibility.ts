/**
 * MA ADU eligibility logic — self-contained copy for the MCP server.
 *
 * Identical algorithm to [frontend/lib/eligibility.ts](../../../frontend/lib/eligibility.ts).
 * The duplication is deliberate: the MCP server runs in its own deployment
 * surface (remote container, claude.ai/directory) and cannot import from
 * frontend without forcing both packages into a monorepo build. The
 * eligibility-parity.test.ts asserts byte-equivalent output for a fixed
 * set of inputs; any future divergence fails CI.
 *
 * State floor: Ch 150 of the Acts of 2024 §§ 7-8, amending MGL Ch 40A
 * §§ 1A and 3, implemented by 760 CMR 71.00 (effective 2025-01-31).
 */
...
export interface EligibilityCitation {
  authority: string;
  source_url: string;
  relevance: string;
  excerpt: string;
...
export function evaluateEligibility(input: EligibilityInput): EligibilityOutput {
  ...
  const citations: EligibilityCitation[] = [
    {
